    ...
    return (
        <div className="card w-96 bg-neutral text-neutral-content">
            <div className="card-body items-start text-center p-2">
                <Link href={'/profile/' +  data.userWithoutPassword.id} className="flex flex-row gap-3">
                    ...
                    <h2 className="card-title text-[0.8rem]">{data && data.userWithoutPassword.fullName}</h2>
                    <p className='text-right text-neutral-content text-[.75rem] ml-10'>{new Date(props.comment.createdAt).toLocaleString()}</p>
                </Link>
