...
export default function Content(props: IProps) {
  ...
  const [isCommentsOpen, toggleIsCommentsOpen, setIsCommentOpen] =
    useToggle(false);
  ...
  useEffect(() => {
    axios
      .get(
        `${process.env.NEXT_PUBLIC_SERVER_URL as string}/likes/posts/${
          props.post.id
        }`
      )
      .then(({ data }) => {
    ...
    if (data) {
      axios
        .get(
          ...
          }/follows/isMyFollowing/${data.userWithoutPassword.id}`
        )
  ...
  const handleFollow = () => {
    axios
      .post(
        `${
          process.env.NEXT_PUBLIC_SERVER_URL as string
        }/follows/followings/me/add/${data.userWithoutPassword.id}`
      )
  ...
  const handleUnFollow = () => {
    axios
      .delete(
        `${
          process.env.NEXT_PUBLIC_SERVER_URL as string
        }/follows/followings/me/delete/${data.userWithoutPassword.id}`
      )
  ...
  const handleLike = () => {
    ...
    axios
      .get(
        `${process.env.NEXT_PUBLIC_SERVER_URL as string}/likes/posts/add/${
          props.post.id
        }`
      )
  ...
  const handleUnLike = () => {
    ...
    axios
      .get(
        `${process.env.NEXT_PUBLIC_SERVER_URL as string}/likes/posts/remove/${
          props.post.id
        }`
      )
      .then((data) => setNumberOfLikes(numberOfLikes - 1));
  ...
  const handleDelete = () => {
    axios
      .delete(
        `${process.env.NEXT_PUBLIC_SERVER_URL as string}/posts/${props.post.id}`
      )
  ...
  return (
    <div
      className="card card-compact sm:max-w-[75%] sm:w-auto w-[100%] bg-base-100 shadow-xl"
      ref={post}
    >
      <div className="card-body flex flex-row gap-5">
        ...
        >
          <div className="avatar" ref={avatar}>
            <div className="w-10 h-10 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2">
              {isMine && user.avatar && session && session!.user && (
                <img
                  src={user.avatar}
              ...
              {!isMine && data && data.userWithoutPassword.avatar && (
                <img
          ...
          <h2 className="card-title sm:text-[1.2rem] text-[1rem] mx-3">
            {data.userWithoutPassword.fullName}
          </h2>
        </Link>
        ...
        {isMine && (
          <div className="dropdown dropdown-end">
            <label
              tabIndex={0}
              className="btn m-1 rounded-full btn-secondary btn-outline btn-sm sm:btn-md"
            >
              <AiOutlineMore size="1.5rem" />
            </label>
            <ul
              tabIndex={0}
      ...
      <div className="card-body flex flex-col flex-wrap">
        ...
        <div className="card-actions justify-start mb-2">
          ...
          <p className="text-right text-neutral-content">
