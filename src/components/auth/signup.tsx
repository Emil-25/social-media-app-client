...
export default function SignUp() {
    ...
    const isPasswordsSame = (passwd: String) => {
        if (passwd === (password.current?.children[0] as HTMLInputElement).value) return true
        return false
    }
                ...
                <form action="" method="POST" onSubmit={handleSubmit(onSubmit)} className="flex flex-col items-center gap-3 w-full">
                    ...
                    <div className="form-control w-full max-w-xs">
                        ...
                        <div ref={password}>
                            ...
                            {errors.password?.type === 'minLength' && <p role="alert" className="text-error mt-1">*Minimum length should be 7 characters</p>}  
                            {errors.password?.type === 'maxLength' && <p role="alert" className="text-error mt-1">*Maximum length should be 30 characters</p>}  
                        </div>
                    </div>
                    ...
                    <div className="form-control w-full max-w-xs">
                        ...
                        <input type="password" 
                            {...register("cpassword", {required: true, validate: isPasswordsSame})} 
                            placeholder="ex. 1ongpa55w0rdw1thnumbers" 
                            className="input input-bordered input-primary w-full max-w-xs" 
                            aria-invalid={errors.cpassword ? "true" : "false"}
                        />
                        {errors.cpassword?.type === 'required' && <p role="alert" className="text-error mt-1">*Confirm your password</p>}
                        {errors.cpassword?.type === 'validate' && <p role="alert" className="text-error mt-1">*Passwords should be same</p>}
                    </div>
