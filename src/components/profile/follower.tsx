  ...
  const handleFollow = () => {
    axios
      .post(
        ...
        }/follows/followings/me/add/${props.user.id}`
      )
  ...
  const handleUnFollow = () => {
    axios
      .delete(
        `${
          process.env.NEXT_PUBLIC_SERVER_URL as string
        }/follows/followings/me/delete/${props.user.id}`
      )
  ...
  useEffect(() => {
    axios
      .get(
        `${
          process.env.NEXT_PUBLIC_SERVER_URL as string
        }/follows/isMyFollowing/${props.user.id}`
      )
      ...
      <Link href={'/profile/' + props.user.id} className="flex flex-row">
        <div className="avatar" ref={avatar}>
          <div className="w-10 h-10 rounded-full">
            ...
            {!props.user.avatar && !(session && session!.user!.image) && (
